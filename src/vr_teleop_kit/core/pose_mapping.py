...
def main() -> None:
    ...
    p, q = out
    print(
        f"T1 (no motion):                target=({p.round(4)}, {q.round(4)})  "
    ...
    print(
        f"T2 (controller +5cm X, R=I):   target_pos={p.round(4)}  expected={expected.round(4)}  "
    ...
    print(
        f"T3 (scale 0.5):                target_pos={p.round(4)}  expected={expected.round(4)}  "
    ...
    print(
        f"T4 (R = Rz90, controller +X):  target_pos={p.round(4)}  expected={expected.round(4)}  "
        f"[{'ok' if _close(p, expected) else 'FAIL'}]"
    )
    ...
    print(
        f"T5 (controller rotates +30° Y, R=I): target_quat={q.round(4)}  "
    ...
    print(
        f"T7 (re-engage at new EE):      target_pos={p.round(4)}  expected={expected.round(4)}  "
        f"[{'ok' if _close(p, expected) else 'FAIL'}]"
    )
    ...
    print(
        f"T8 (rot reach limit, 120° push):     max target-vs-EE angle={worst:.1f}°  "
    ...
    print(
        f"T9 (reversal bites at reach limit):  residual after backing off {np.degrees(0.5):.1f}°: "
    ...
    p, _ = m10.target(
        ctrl_engage_pos + np.array([1.0, 0, 0]), ctrl_engage_quat, ee_engage_pos, ee_engage_quat
    )
    ok1 = _close(p, ee_engage_pos + np.array([0.25, 0, 0]))
    p, _ = m10.target(
        ctrl_engage_pos + np.array([0.95, 0, 0]), ctrl_engage_quat, ee_engage_pos, ee_engage_quat
    )
    ok2 = _close(p, ee_engage_pos + np.array([0.20, 0, 0]))
    print(
        f"T10 (pos reach limit, mouse-style):  clamp@1m={'ok' if ok1 else 'FAIL'}  "
    ...
    print(
        f"T11 (incremental == absolute within reach limit): max diff={worst:.4f}°  "
