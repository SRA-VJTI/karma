...
wraps the bimanual teleop and strips the prefix for one chosen arm. Useful for
DAgger interventions on a policy trained on a single arm: the operator can
still wear both Quest controllers (one drives,
the other is idle / used for the B/Y handoff button), but the action dict
landing on the robot matches the single-arm schema.
...
class SingleArmQuestTeleoperator(Teleoperator):
    ...
    def get_action(self) -> dict[str, float]:
        ...
        return {
            k.removeprefix(self._prefix): v for k, v in full.items() if k.startswith(self._prefix)
        }
