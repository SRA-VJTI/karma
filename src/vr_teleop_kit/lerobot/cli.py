"""Shared IK and mapping helpers for the openpi VR teleop entry points.

The unified ``openpi teleop`` command owns the hardware loop; this module keeps
the reusable option definitions and environment parsing that the original kit
also exposed to its example programs.
"""
...
_IK_CLI_FIELDS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "--rot-reach",
        "rot_reach_limit",
        "Max angle (rad) the orientation target may run ahead of the arm; 0 disables.",
    ),
    (
        "--pos-reach",
        "pos_reach_limit",
        "Max distance (m) the position target may run ahead of the arm; 0 disables.",
    ),
    (
        "--rot-err-hold",
        "rot_err_hold",
        "Park the wrist above this orientation error (rad); > 3.15 disables (demo use).",
    ),
    ...
    (
        "--pose-filter-alpha",
        "pose_filter_alpha",
        "EMA on the controller pose (1.0 = none, lower = smoother + laggier).",
    ),
)
...
def add_ik_cli_args(ap: argparse.ArgumentParser) -> None:
    ...
    for flag, _, help_text in _IK_CLI_FIELDS:
        ap.add_argument(
            ...
            type=float,
            default=None,
            help=help_text + " (see BiQuestTeleoperatorConfig for default)",
        )
...
def ik_kwargs_from_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        for _, dest, _ in _IK_CLI_FIELDS
