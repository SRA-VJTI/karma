...
class BiQuestTeleoperator(Teleoperator):
        ...
        with self._lock:
            for hand in ("left", "right"):
                ...
                if self._haptic_calib is not None:
                    self._haptic_calib["peak"][hand] = max(self._haptic_calib["peak"][hand], tau)
    ...
    def _update_arm(
        self, hand: str, ctrl: dict | None, yaw_now: float | None, gap_s: float
    ) -> None:
        if ctrl is None:
            return
        arm = self._arms[hand]
        ...
        if grip != arm["last_grip"]:
            logger.info(
                "%s grip button %s (engaged=%s)",
                hand,
                "PRESSED" if grip else "released",
                arm["engaged"],
            )
        trigger = (
            float(buttons[TRIGGER_BUTTON_INDEX]["v"])
            if len(buttons) > TRIGGER_BUTTON_INDEX
            else 0.0
        )
        precision = (
            bool(buttons[PRECISION_BUTTON_INDEX]["p"])
            if len(buttons) > PRECISION_BUTTON_INDEX
            else False
        )
        ...
        rest_btn = (
            bool(buttons[REST_RAMP_BUTTON_INDEX]["p"])
        ...
        if grip and not arm["last_grip"]:
            ...
            if not arm["last_grip"]:
                ...
                logger.warning(
                    ...
                    "(missed release edge) — force-disengaging",
                    hand,
                )
            arm["mapper"].disengage()
            arm["engaged"] = False
    ...
    def _finalize_haptic_calibration(self) -> None:
        ...
        if suspicious:
            logger.warning(
                ...
                ", ".join(suspicious),
                self._HAPTIC_CALIB_SUSPICIOUS_PEAK_NM,
            )
    ...
    async def _ws_runner(self) -> None:
        ...
        while not self._ws_stop.is_set():
            try:
                        ...
                        if mtype == "xr_frame":
                            ...
                            for hand in ("left", "right"):
                                ...
                                if len(buttons) > GRIP_BUTTON_INDEX:
                                    btn_snapshot[hand]["grip"] = bool(
                                        buttons[GRIP_BUTTON_INDEX].get("p")
                                    )
                ...
                logger.warning(
                    "BiQuestTeleoperator WS error (%s); reconnecting in %.1fs",
                    type(e).__name__,
                    backoff,
                )
            finally:
