...
class DecoupledIKSolver:
    ...
    def __init__(
        ...
        self.q_rest_123 = (
            ...
            if q_rest is None
            else np.asarray(q_rest, dtype=float)[:3].copy()
        )
    ...
    def solve(
        ...
        self.last_limit_pressure = float(np.linalg.norm(qpos6 - qpos6_reachable))
        if wrist_parked:
...
def _self_test() -> None:
    ...
    print(
        f"  far-out-of-reach: qpos6 finite={bool(np.all(np.isfinite(out)))}  "
    ...
    print(f"  wrist near gimbal: qpos6 finite={bool(np.all(np.isfinite(out)))}  θ5={out[4]:.4f}")
    ...
    print(
        f"  gimbal conditioning @θ5=88°: worst J4/J6 step for 1° twist = "
    ...
    print(
        f"  convergence (100 iters): rot_err={rot_err_deg:.3f}°  "
    ...
    print(
        f"  antipode gate: parked@150°={parked} (pressure={pressure_at_park:.2f})  "
