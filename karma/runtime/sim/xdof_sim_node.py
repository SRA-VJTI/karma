...
- Recording: {name}-left.mcap + {name}-right.mcap + {name}-sim_state.mcap +
  per-camera MP4s (configs use name: sim)
...
Subscribed topics (from cmd_topics):
    e.g. "leader_left/joint_pos", "leader_right/joint_pos"
"""
...
from karma.runtime.node import Node, NodeRole
...
class _VrStreamer:
    ...
    def __init__(
        ...
        from karma.runtime.sim._mujoco_viser import VR_HTML, export_body_glbs
        ...
        mesh_dir = Path(f"/tmp/karma_vr_{task or 'sim'}")
        if mesh_dir.exists():
