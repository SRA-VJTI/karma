"""Viser replay viewer for karma sim episode recordings.

Loads an episode directory recorded by karma and replays it in an
interactive Viser browser UI.  Two modes are available:
  ...
  qpos    — directly restore the recorded MuJoCo qpos at each step (exact replay,
             requires sim-sim_state.mcap; preferred for sim episodes).
  physics — feed the recorded leader actions back through step_single (useful for
             checking policy/action consistency).

Both modes run at the control rate (~30 Hz with default physics_dt=0.002, decimation=17).

Usage:
    krm replay recordings/20260323/episode_*/
    krm replay <episode_dir> --port 8080 --speed 2.0
    krm replay <episode_dir> --scene hybrid --task bottles
"""
...
def _decode_mcap_payload(channel, msg) -> dict | None:  # noqa: ANN001
    """Decode Karma MCAP payloads (JSON fallback plus RobotState protobuf)."""
    try:
    ...
    if topic.endswith("-robot-state"):
        try:
            from xdof_sdk.proto.robot_state_pb2 import RobotState

            state = RobotState()
            state.ParseFromString(msg.data)
            data: dict[str, list[float]] = {}
            if state.position:
                data["joint_pos"] = list(state.position)
            if state.velocity:
                data["joint_vel"] = list(state.velocity)
            if state.torque:
                data["joint_eff"] = list(state.torque)
            return data
        except Exception:
            return None
    return None
...
def _read_mcap_records(path: Path) -> list[tuple[float, dict]]:
    """Return [(ts_seconds, data_dict), ...] from a Karma MCAP file."""
    from mcap.reader import make_reader

    results: list[tuple[float, dict]] = []
    with open(path, "rb") as f:
        for _, channel, msg in make_reader(f).iter_messages():
            data = _decode_mcap_payload(channel, msg)
            if data is None:
                continue
...
def _first_existing(episode_dir: Path, names: tuple[str, ...]) -> Path | None:
    for name in names:
...
def _load_episode(episode_dir: Path) -> dict:
    """Load all streams from a karma sim episode directory.
    ...
    # --- Actions from leader MCAP files ---
    def _load_arm(names: tuple[str, ...], label: str):
        path = _first_existing(episode_dir, names)
        if path is None:
            raise FileNotFoundError(f"Missing one of {names} in {episode_dir}")
        msgs = [(t, d) for t, d in _read_mcap_records(path) if "joint_pos" in d]
        if not msgs:
            raise ValueError(f"No joint_pos samples found in {path}")
    ...
    actions_left,  ts_left  = _load_arm(("leader_left.mcap",),  "leader_left")
    actions_right, ts_right = _load_arm(("leader_right.mcap",), "leader_right")

    # --- Camera videos ---
    camera_frames: dict[str, np.ndarray] = {}
    camera_ts:     dict[str, np.ndarray] = {}
    for cam in ("left", "right", "top"):
        mp4 = _first_existing(episode_dir, (f"sim-{cam}-images-rgb.mp4",))
        npy = _first_existing(episode_dir, (f"sim-{cam}-rgb-timestamp.npy",))
        if mp4 is None:
            continue
        try:
            import imageio.v3 as iio
            frames = iio.imread(str(mp4), plugin="pyav")  # (T, H, W, 3)
            ts_arr = np.load(str(npy)) if npy is not None and npy.exists() else np.linspace(ts_left[0], ts_left[-1], len(frames))
...
def _load_sim_states(episode_dir: Path):
    """Load full qpos timeline from sim-sim_state.mcap.

    Returns (qposes, timestamps) or None if file is absent.
    """
    mcap_file = _first_existing(episode_dir, ("sim-sim_state.mcap",))
    if mcap_file is None:
        return None
    msgs = _read_mcap_records(mcap_file)
...
class EpisodeReplayViewer:
    """Interactive Viser 3D viewer for replaying karma sim episodes.
    ...
    def __init__(
        ...
        *,
        ...
        from karma.runtime.sim._mujoco_viser import (
            _get_body_name,
            _is_fixed_body,
            _merge_geoms,
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Replay a karma sim episode through MuJoCo + Viser"
    )
    ...
    if raw_states is not None:
        ...
        print("No sim-sim_state.mcap — only physics re-step mode available")
