...
4. Serve that checkpoint and verify its health contract; use `karma inference`
   for a supervised first run or `karma rollout` for timed recorded attempts.
...
an intervention label. Training, evaluation criteria and GPU model serving are
